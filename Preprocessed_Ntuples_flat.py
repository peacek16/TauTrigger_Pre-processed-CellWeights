...
FLEt = open("PreLEtTau_11x3_ViEt15Eta1.4_NNTau_Train.data","w")
...
for entry in signal:
    ...
    if seed.Et()>5000 and vipt>15000 and -1.4<seed.Eta()<1.4:
        ...
        FLEt.write(','.join(str(L0cells_2D[i][j]) for i in range(1,4) for j in range(3,14))+',')
        FLEt.write(','.join(str(L1cells_2D[i][j]) for i in range(1,4) for j in range(3,14))+',')
        FLEt.write(','.join(str(L2cells_2D[i][j]) for i in range(1,4) for j in range(3,14))+',') 
        FLEt.write(','.join(str(L3cells_2D[i][j]) for i in range(1,4) for j in range(3,14))+',')
        FLEt.write(','.join(str(Hadcells_2D[i][j]) for i in range(1,4) for j in range(3,14))+',')
