...
"""
    File ouputs: 12 Weight maps of epoch with acc < 0.001, acc plot, and resolution distribution
...
dataset = open("PreLEtTau_12_ViEt15Et5Eta1.4_W.data","r")
...
for line in dataset:
    ...
    if math.fabs(float(ls_line[-1])) < 0.001:       # optional; only print epoch with net-true/true < 0.001
        ...
        pyplot.title("Pre-processed LEt 12 Weights: Adam Rate = 1, Epoch = "+str(run))
        pyplot.axis('off')
        ...
        pyplot.savefig("PreLEt_12_EvalWeight_Run"+str(run)+".pdf")
...
g.SetTitle("Pre-processed LEt 12 weights : Adam Rate = 1, Epoch = 300 and Batch size = 500")
...
canvas["EvalAcc"].SaveAs("PreLEt_12_Weights_EvalAcc.pdf")
...
hist.SetTitle("Pre-processed LEt 12 weights : Adam Rate = 1, Epoch = 300 and Batch size = 500") 
canvas["EvalResolution"].SaveAs("PreLEt_12_EvalResolution.pdf")
